...
import shutil  # To use shutil.move for moving files
...
remote_path = "~/Documents/Workspace/Granos/FTP/Data_luke"
                ...
                while not upload_success and attempt_count < 3:
                    try:
                        ...
                        if attempt_count < 3:
                            ...
                            # Save to the failed directory after 3 unsuccessful attempts
                            log_with_time(logger, f"Saving {last_jpg_file} to failed upload directory")
                            print(f"Saving {last_jpg_file} to failed upload directory")
                            shutil.move(last_jpg_file, os.path.join(failed_upload_dir, os.path.basename(last_jpg_file)))
                ...
                while not upload_success and attempt_count < 3:
                    try:
                        ...
                        if attempt_count < 3:
                            ...
                            # Save to the failed directory after 3 unsuccessful attempts
                            log_with_time(logger, f"Saving {last_csv_file} to failed upload directory")
                            print(f"Saving {last_csv_file} to failed upload directory")
                            shutil.move(last_csv_file, os.path.join(failed_upload_dir, os.path.basename(last_csv_file)))
