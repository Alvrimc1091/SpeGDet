...
def mostrar_datos():
    ...
    foto_id = f"foto_{hora_santiago.strftime('%d%m%Y_%H%M%S')}.jpg"
...
def guardar_datos(datos, foto_id, hora_santiago):
    ...
    with open(f"/home/pi/SpeGDet/DataMeassures/TestMeassures/data_proto2_{hora_santiago.strftime('%d%m%Y_%H%M%S')}.csv", mode='a', newline='') as archivo_csv:
...
def tomar_foto():
    try:
        picam2.start()    
        hora_santiago = datetime.datetime.now(zona_santiago)
        nombre_foto = f"/home/pi/SpeGDet/DataMeassures/TestMeassures/foto_{hora_santiago.strftime('%d%m%Y_%H%M%S')}.jpg"
        fecha_hora_actual = hora_santiago.strftime("%d%m%Y_%H%M%S")
